...
def _get(path: str, params: dict | None = None):
    if not _HAS_HTTPX:
        return None
    try:
        r = httpx.get(f"{API_BASE}{path}", params=params or {}, timeout=15.0)
        r.raise_for_status()
        return r.json()
...
def _put(path: str, params: dict):
    if not _HAS_HTTPX:
        return None
    try:
        r = httpx.put(f"{API_BASE}{path}", params=params, timeout=15.0)
        r.raise_for_status()
        return r.json()
    except Exception as exc:  # noqa: BLE001
...
with tab5:
    ...
    if not df_dq.empty:
        ...
        def _style(val):
            return {"PASS": "color:#10b981", "WARNING": "color:#f59e0b",
                    "FAIL": "color:#ef4444"}.get(val, "")
