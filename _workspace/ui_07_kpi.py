...
COLOR_HEX = {"green": "#00c853", "yellow": "#ffd600", "orange": "#ff9800", "red": "#f44336"}
...
def gauge_figure(item: dict) -> go.Figure:
    ...
    color = COLOR_HEX.get(item.get("color", "red"), "#f44336")
...
with tab_status:
    ...
    if summary and summary.get("items"):
        ...
        def _row_color(row):
            ...
            bg = {"green": "#0f3d23", "yellow": "#4a4012", "orange": "#4a3012", "red": "#4a1212"}.get(it["color"], "")
...
with tab_prod:
    ...
    if trend and trend.get("data"):
        ...
        fig.add_trace(go.Scatter(
            ...
            mode="lines+markers", name="시간당 생산량", line=dict(color="#1a73e8")))
        fig.add_hline(y=target, line_dash="dash", line_color="#ff9800",
        ...
        fig.update_layout(height=420, yaxis_title="kg/h", xaxis_title="기간",
...
with tab_qual:
    ...
    with col_l:
        ...
        if dtrend and dtrend.get("data"):
            ...
            fig.update_layout(height=380, yaxis_title="%", xaxis_title="기간",
                              legend=dict(orientation="h", y=-0.2))
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    with col_r:
        ...
        if by_proc:
            ...
            if not agg.empty and agg["defect_count"].sum() > 0:
                ...
                fig.update_layout(height=380, legend=dict(orientation="h", y=-0.1))
                st.plotly_chart(fig, use_container_width=True)
