...
def api_post(path: str, json: dict, params: dict | None = None):
    try:
        r = httpx.post(f"{BASE_URL}{path}", json=json, params=params or {}, timeout=10.0)
        r.raise_for_status()
        return r.json()
...
def api_patch(path: str, json: dict):
    try:
        r = httpx.patch(f"{BASE_URL}{path}", json=json, timeout=10.0)
        r.raise_for_status()
        return r.json()
    except Exception as e:  # noqa: BLE001
    ...
    if analysis:
        ...
        with col_b:
            ...
            if daily:
                ...
                fig2.add_hline(y=1.0, line_dash="dash", line_color="#10b981",
